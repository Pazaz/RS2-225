...
public class VarpDump {
    ...
    public static void main(String[] args) {
        try {
            ...
            VarpType.decode(config225);

            try {
                PrintWriter out = new PrintWriter("dump/varp.def");

                for (int i = 0; i < VarpType.count; ++i) {
                    String config = VarpType.instances[i].toJagConfig();
                    if (config.length() > 0) {
                        out.println(VarpType.instances[i].toJagConfig());
                    }
                }
