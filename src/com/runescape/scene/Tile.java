package com.runescape.scene;

import com.runescape.util.Node;

public class Tile extends Node {

    public Tile(int i, int j, int k) {
        aClass28Array1172 = new Loc[5];
        anIntArray1173 = new int[5];
        anInt1164 = anInt1161 = i;
        anInt1162 = j;
        anInt1163 = k;
    }

    public int anInt1161;
    public int anInt1162;
    public int anInt1163;
    public int anInt1164;
    public TileUnderlay aClass26_1165;
    public TileOverlay aClass15_1166;
    public Wall aClass30_1167;
    public WallDecoration aClass13_1168;
    public GroundDecoration aClass17_1169;
    public ObjEntity aClass19_1170;
    public int anInt1171;
    public Loc[] aClass28Array1172;
    public int[] anIntArray1173;
    public int anInt1174;
    public int anInt1175;
    public boolean aBoolean1176;
    public boolean aBoolean1177;
    public boolean aBoolean1178;
    public int anInt1179;
    public int anInt1180;
    public int anInt1181;
    public int anInt1182;
    public Tile aClass38_Sub1_1183;
}
