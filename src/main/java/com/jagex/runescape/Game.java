...
public class Game extends GameShell {
    ...
    public void updateComponentContent(InterfaceComponent interfaceComponent) {
        ...
        if (j == 600) {
            interfaceComponent.text = reportInput;
        ...
        if (j == 651) {
            ...
            if (unreadMessageCount > 1) {
                interfaceComponent.text = unreadMessageCount + " unread messages";
                interfaceComponent.color = 65280;
            }
        }
        if (j == 652)
        ...
        try {
            ...
            if (packetOpcode == Packet.LOAD_AREA) {
                ...
                for (int n = 0; n < mapCount; n++) {
                    ...
                    if (landCrc != 0) {
                        byte[] data = Signlink.cacheload("m" + x + "_" + y);
                        if (data != null) {
                            crc32.reset();
                            crc32.update(data);
                    ...
                    if (locCrc != 0) {
                        byte[] abyte2 = Signlink.cacheload("l" + x + "_" + y);
                        if (abyte2 != null) {
                            crc32.reset();
                            crc32.update(abyte2);
    ...
    public void updatePlayerMask(int i, int mask, Buffer b, PlayerEntity p) {
        ...
        if ((mask & 2) == 0x2) {
            ...
            if (anim == -1 || p.primarySeq == -1 || SeqType.animations[anim].priority > SeqType.animations[p.primarySeq].priority || SeqType.animations[p.primarySeq].priority == 0) {
                p.primarySeq = anim;
                p.primarySeqFrame = 0;
                p.primarySeqCycle = 0;
