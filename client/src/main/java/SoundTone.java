...
public final class SoundTone {
	...
	public final int[] generate(@OriginalArg(0) int sampleCount, @OriginalArg(1) int length) {
	...
	public final void read(@OriginalArg(1) Buffer buffer) {
		...
		for (@Pc(122) int i = 0; i < 10; i++) {
			@Pc(132) int volume = buffer.gSmart1or2();
			if (volume == 0) {
				break;
			}
			this.harmonicVolume[i] = volume;
			this.harmonicSemitone[i] = buffer.gSmart1or2s();
			this.harmonicDelay[i] = buffer.gSmart1or2();
		}
		this.reverbDelay = buffer.gSmart1or2();
		this.reverbVolume = buffer.gSmart1or2();
