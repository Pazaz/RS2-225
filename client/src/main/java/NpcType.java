...
public final class NpcType {
	...
	public static void decode(@OriginalArg(0) FileArchive arg0) {
		dat = new Buffer(363, arg0.read("npc.dat", null));
		@Pc(21) Buffer idx = new Buffer(363, arg0.read("npc.idx", null));
		count = idx.g2();
	...
	private void decode(@OriginalArg(1) Buffer buffer) {
		while (true) {
			...
			if (opcode == 1) {
				...
				this.options[opcode - 30] = buffer.gjstr();
	...
	public final Model getModel(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int[] arg2) {
		...
		if (local9 == null) {
			...
			for (@Pc(18) int local18 = 0; local18 < this.modelIndices.length; local18++) {
				local16[local18] = new Model(false, this.modelIndices[local18]);
			}
			if (local16.length == 1) {
				local9 = local16[0];
			} else {
			...
			if (this.oldColors != null) {
				for (@Pc(60) int local60 = 0; local60 < this.oldColors.length; local60++) {
					local9.recolor(this.oldColors[local60], this.newColors[local60]);
				}
			}
			local9.applyGroup();
		...
		@Pc(107) Model local107 = new Model(0, local9, !this.disposeAlpha);
		if (arg0 != -1 && arg1 != -1) {
			local107.applyFrames(arg1, arg0, arg2);
		} else if (arg0 != -1) {
			local107.applyFrame(arg0);
		}
		if (this.scaleX != 128 || this.scaleZ != 128) {
			local107.scale(this.scaleX, this.scaleZ, this.scaleX);
		}
	...
	public final Model getHeadModel() {
		...
		for (@Pc(19) int local19 = 0; local19 < this.headModelIndices.length; local19++) {
			local17[local19] = new Model(false, this.headModelIndices[local19]);
		}
		@Pc(46) Model local46;
		if (local17.length == 1) {
			local46 = local17[0];
		...
		if (this.oldColors != null) {
			for (@Pc(61) int local61 = 0; local61 < this.oldColors.length; local61++) {
				local46.recolor(this.oldColors[local61], this.newColors[local61]);
			}
		}
		return local46;
	}
