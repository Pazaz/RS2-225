...
public class HTTPServer {
    ...
    public static class VirtualHost {
        ...
        protected final SortedMap<String, ContextInfo> contexts =
            new TreeMap<String, ContextInfo>();
        ...
        public ContextInfo getContext(String path) {
            ...
            for (path = trimRight(path, '/'); path != null; path = getParentPath(path)) {
                ...
                for (Map.Entry<String, ContextInfo> entry : contexts.entrySet()) {
                    // skip anything without a pattern key
                    if (!entry.getKey().contains("${")) {
                        continue;
                    }

                    // TODO: now that we have the context, we should preserve the pattern matched as a param
                    int endOfUri = entry.getKey().indexOf("${");
                    if (path.startsWith(entry.getKey().substring(0, endOfUri))) {
